...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/01/19 03:56:11 by artainmo          #+#    #+#             */
/*   Updated: 2020/01/19 04:07:08 by artainmo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_user_guide(void)
{
	...
	ft_putstr_fd("n:next(up or down)\n o:orientation\n", 0);
	ft_putstr_fd("Type a number, default 10, ", 0);
	ft_putstr_fd("use arrows to increase, decrease or move(first number)\n", 0);
	ft_putstr_fd("type backslach to command\n", 0);
	ft_putstr_fd("Access different objects\n Type v,", 0);
	ft_putstr_fd(" iterate number obj (if your number is too high it ", 0);
	ft_putstr_fd("goes back to the first ob)\nafter type the commands ", 0);
	ft_putstr_fd("you want to execute on that ob (no number here)\n", 0);
...
static t_s		*window_setup(t_s *s)
{
	if (!(s->screen.mlx_ptr = mlx_init()))
		ft_exit_no_free("ERROR\nSoftware display connexion failed\n");
