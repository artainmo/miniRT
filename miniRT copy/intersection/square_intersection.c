...
*/
static int ft_inter_in_sq(t_vec centrum, double side, t_vec dir, t_vec p)
{
  t_vec a;
  t_vec b;
  t_vec c;
  t_vec x_axis = ft_vector_cross_product(ft_point_vector(0,1,0), dir);
  t_vec y_axis = ft_vector_cross_product(ft_point_vector(1,0,0), dir);
...
int square_intersection_boolean(t_s *s, t_ray ray, t_inter *inter)
{
  ...
  if (inter->shadow.dis1 != INFINITY)
  {
    ...
    return (ft_inter_in_sq(s->sq.p, s->sq.side,  s->sq.ndir, inter->shadow.p));
  }
  return (0);
}
...
t_inter *ft_square_intersection(t_s *s, t_inter *inter, t_ray ray, t_light l)
{
  ...
  if (inter->dis1 != INFINITY)
  {
    ...
    if (ft_inter_in_sq(s->sq.p, s->sq.side, s->sq.ndir, inter->p))
      inter = ft_intersection_color(s, inter, l, s->sq.c);
    else
      inter->dis1 = INFINITY;
  }
  return (inter);
...
static int ft_inter_in_sq(t_vec centrum, double side, t_vec dir, t_vec p)
{
  ...
  t_vec x_axis = ft_vector_cross_product(ft_point_vector(0, 1, 0), dir);
  t_vec y_axis = ft_vector_cross_product(dir, x_axis);

  xl = ft_vec_move(centrum, x_axis, -side/2);
  xr = ft_vec_move(centrum, x_axis, +side/2);
  yu = ft_vec_move(centrum, y_axis, +side/2);
  yd = ft_vec_move(centrum, y_axis, -side/2);
  if (p.x >= xl.x - ERROR_MARG && p.x <= xr.x + ERROR_MARG &&
        p.y >= yd.y - ERROR_MARG && p.y <= yu.y + ERROR_MARG)
    return (1);
  else
    return (0);
}

static int ft_inter_in_sq(t_vec centrum, double side, t_vec p)
{
  double xl;
  double xr;
  double yu;
  ...
  yu = centrum.y + side/2;
  yd = centrum.y - side/2;
  if (p.x >= xl - ERROR_MARG && p.x <= xr + ERROR_MARG &&
        p.y >= yd - ERROR_MARG && p.y <= yu + ERROR_MARG)
    return (1);
  else
    return (0);
}
