...
static t_col ft_diffuse_col(t_s *s, t_light l, t_inter *inter, t_col o_c)
{
  t_col diffuse;
	...
	if ((angle = 1 - ft_vector_dot_product(inter->n, l.ray.direc)) < 0
				|| inter->p.z - s->camera.p.z
            < l.position.z - s->camera.p.z)
		angle = 0;
  diffuse.red = o_c.red * (angle * l.color.red/255);
  diffuse.green = o_c.green * (angle * l.color.green/255);
  diffuse.blue = o_c.blue * (angle * l.color.blue/255);
